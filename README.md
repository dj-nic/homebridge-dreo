...
* **Target Humidity:** Adjust the drying target between 30% and 85% using the slider, and read back the exact setpoint via a separate "Target Humidity" sensor tile (this sensor is hidden by default but can be shown by setting `hideTargetHumiditySensor: false` in the config).
...
* **Panel Sound / Display:** The "Panel Sound" switch (hidden by default, can be shown by setting `hidePanelSoundSwitch: false` in the config) controls the device buzzer, and the "Display Light" switch controls the on-device display.
